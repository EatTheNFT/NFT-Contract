...
// File @openzeppelin/contracts-upgradeable/token/ERC721/IERC721Upgradeable.sol@v4.4.2
...
// File @openzeppelin/contracts-upgradeable/token/ERC721/extensions/IERC721MetadataUpgradeable.sol@v4.4.2
...
// File contracts/EatTheNFTUpgradeable.sol
    ...
    OwnableUpgradeable
{
    ...
    address private constant EatTheNFT =
    ...
    function __EatTheNFT_init(
        string memory _name,
        string memory _symbol,
        uint256 _maxSupply
    ) public initializer onlyInitializing onlyOwner {
    ...
    function setCommission(uint256 _val1) public {
        require(msg.sender == EatTheNFT, "Invalid address");
        uint256 diff = _shares[EatTheNFT] - _val1;
        _shares[EatTheNFT] = _val1;
    ...
    function updateReferral(uint256 _val) public onlyOwner {
        referralOpen = true;
        referralCap = _val;
    }

       function updateRoyalties(address _address)
        public
        onlyOwner
    {
        recipient = _address;
    ...
    function mintAll() external payable onlyOwner {
        if (msg.value > 0) {
            AddressUpgradeable.sendValue(payable(EatTheNFT), msg.value);
        }

        _safeMint(owner(), totalSupply() - supply());
    }

    function presaleMint(uint32 count)
        external
        payable
        preMintChecks(count)
    {
